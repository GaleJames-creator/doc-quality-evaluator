...
The dash rules have three deliberate exceptions, so the checker doesn't fight the way Markdown is actually written:

* **Headings** are exempt entirely. A heading generates its own anchor, and other pages link to it.
* **A table cell holding only a dash** keeps its surrounding spaces, since `| &mdash; |` is how a "not applicable" cell is written.
* **Repeated digit groups** don't count as repeated words, so `4242 4242 4242 4242` in a test card table reads as data rather than a typo.

It also flags stray periods in table cells. If your operating system substitutes `. ` for a double space, padding a cell to align its pipes silently inserts a period, producing a header that reads "Type." or a Required column containing `No.`, text a spell checker accepts because a period is valid. On macOS the setting is **System Settings → Keyboard → Text Input → Input Sources → Edit → Add period with double-space**. Turning it off stops new ones; the check finds those already in the file.
